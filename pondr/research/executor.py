...
SYSTEM = (
    ...
    "Quant tools available: run_backtest(strategy_name, symbol) — built-in "
    "strategies are 'ma_cross', 'mean_reversion', 'breakout'; symbols come "
    "from live ticks (e.g. BTCUSDT, BTC-USD). For BESPOKE strategies, prefer "
    "the strategy-synthesis flow: design_strategy(name, hypothesis) writes "
    "fresh on_tick code from your hypothesis, run_strategy(strategy_id, symbol) "
    "runs it on real ticks and returns metrics, iterate_strategy(parent_id, "
    "modification_request) creates a child variant, compare_strategies(ids) "
    "lists them side by side. Use these whenever a finding suggests a new "
    "market regularity worth testing — don't shoehorn into the 3 templates. "
    "query_arb_history(symbol, min_spread_bp, "
